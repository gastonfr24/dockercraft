#!/bin/bash
# Script para agregar jugadores a la whitelist
...
echo "Agregando $PLAYER_NAME a la whitelist..."

# Agregar via RCON
docker exec "$CONTAINER_NAME" rcon-cli whitelist add "$PLAYER_NAME"

# Backup del whitelist
docker cp "$CONTAINER_NAME":/data/whitelist.json ./whitelist.json 2>/dev/null

echo "✅ $PLAYER_NAME agregado exitosamente"
echo ""
echo "Para verificar:"
